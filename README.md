...
AIがコードを書く時代に、エンジニアの仕事はどう変わるのか——ゆむ（エンジニア）とさばきゃん（FP）がAIエージェントと一緒に実際に動くものを作りながら、その問いと向き合っています。

「何を作るか」はドメイン知識が決める。「これで正しいか」はエンジニアリングの基礎力が決める。「どう実装するか」はAIエージェントが担う——その実録のサンプルコードです。

技術解説でも入門講座でもなく、一緒に試しています。
...
| shorts: AIが書いたコード、動いた。1行消したら、止まった。 | 検証関数を読み込む最小Pythonコード。import 1行を削除したコピーを実行し、成功と `NameError` の両方を記録する | Python（標準ライブラリ） | [ai-1gyou-keshita-tomatta](./shorts/ai-1gyou-keshita-tomatta/) |
| shorts: ライブラリを1個上げた。関係ない場所が壊れた。 | 共通入力チェックを更新した一時コピーで、変更していないログインが最小文字数規則により失敗する様子を記録する | Python（標準ライブラリ） | [library-1ko-koware](./shorts/library-1ko-koware/) |
...
| [Claude Code hooks 設計パターン](./notes/claude-code-hooks-design.md) | PreToolUse で変更前にブロック・PostToolUse で自動テスト — CLAUDE.md との使い分けと落とし穴 |
